...
@Suite struct SpendLedgerTests {
    @Test func recordingUpdatesTheSummaryAndLocalCallsCostNothing() {
        ...
        ledger.record(provider: .openRouter, model: "m", usage: Spend.Usage(promptTokens: 10, completionTokens: 5, cost: 0.001), purpose: .voice)
        ledger.record(provider: .lmStudio, model: "local", usage: Spend.Usage(promptTokens: 10, completionTokens: 5, cost: nil), purpose: .talk)
        ...
        #expect(ledger.summary.byPurpose.map(\.purpose) == ["Voice", "Talk"])
        #expect(ledger.recent.map(\.model) == ["local", "m"], "newest first")
        #expect(ledger.recent.first?.purpose == "talk")
