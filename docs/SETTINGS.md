...
| **Server** / **Model** / **Voice** (Local server) | `http://localhost:8880`, `kokoro`, the server's first | | Any speech server on this Mac that answers like OpenAI's `/v1/audio/speech` and lists voices at `/v1/audio/voices`: Kokoro-FastAPI, LMS Speaks. Free; nothing kept or priced. **Check** lists the voices; **Copy Setup Command** puts Kokoro-FastAPI's install-and-start line on the clipboard (`git clone https://github.com/remsky/Kokoro-FastAPI.git ~/Kokoro-FastAPI; cd ~/Kokoro-FastAPI && ./start-gpu_mac.sh`, needs git and uv, downloads about a gigabyte once). LM Studio cannot speak: its server has no speech endpoint |
