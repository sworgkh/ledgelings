...
| **API key** (OpenRouter) | | | Kept in the keychain on macOS and the Credential Manager on Windows, never in the settings file. The app only reads it once OpenRouter is the chosen brain and something needs it, so picking LM Studio never touches the keychain. macOS asks once per new build whether Ledgelings may read it; **Always Allow** stops it asking for that build. Writing the key never asks |
